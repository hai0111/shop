...
const renerHightlight = (() => {
	...
	const result = [...data.product]
		...
		.slice(0, 5)
	result.forEach((a, i) => {
		if (i === 0) highLightBox.innerHTML = ''
		const img = new Image()
		img.src = a.img
		img.onload = function () {
			highLightBox.innerHTML += `<a href="./detail.html?${data.product.indexOf(
				a
			)}" class="txt-main position-relative hover hover-shadow-fade radius-10 mb-5 box-shadow-fade hover__introduce index-1">
        ${
					a.tag && a.tag !== ''
						? '<span class="product__tag px-3 index-1">' + a.tag + '</span>'
						: ''
				}
        <img src="${a.img}"
        ...
        <div
            ...
            <span class="product__rate mt-2 fz-2d5r width-min" style="background: linear-gradient(90deg, rgb(255, 208, 0) ${
							(a.rating / 5) * 100
						}%, #999 ${(a.rating / 5) * 100}%);
...
const renerSale = (() => {
	...
	const result = [...data.product]
        ...
        <div class="position-absolute top-0 end-0 index-1" style="width: 40px;">
            ...
            <span class="position-absolute top-50 start-50 translate-middle fw-600 txt-fff">${
							a.sale
						}%</span>
        </div>
        <img src="${a.img}"
        ...
        <div
            ...
            <p class="fz-2r mb-0">${toThousand(
							a.price * (1 - a.sale / 100)
						)}VNĐ</p>
            <span class="product__rate fz-2d5r width-min" style="background: linear-gradient(90deg, rgb(255, 208, 0) ${
							(a.rating / 5) * 100
						}%, #999 ${(a.rating / 5) * 100}%);
-webkit-background-clip: text;
-webkit-text-fill-color: transparent;">★★★★★</span>
...
const handleHover = (() => {
	...
	saleItems.forEach((a) => {
		a.addEventListener('mouseenter', (e) => {
			const tag = e.target.querySelector(
				'.position-absolute.top-0.end-0.index-1'
			)
			tag.animate(
				[
					{
